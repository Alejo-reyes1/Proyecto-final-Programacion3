defmodule GestionTeams do
  ...
  def createTeam(nombre, usuario,tema) do
    ...
    |> case do
      {:ok, msg} -> asociar_usuario_team(nombre, usuario)
                    asociar_team_usuario(nombre, usuario)
                    {:ok, msg}
      {:error, msg} -> {:error, msg}
    end
  end

  def asociar_usuario_team(nombre_team, participante) do
  ...
  def asociar_team_usuario(nombre_team, participante) do
    ...
    |> case do
      ...
      {:error, _msg} -> teams=TeamRepository.listarTeams()
      team=Enum.find(teams, fn t ->String.trim(t.nombre)==String.trim(nombre_team)end)
      if participante.id in team.participantes do
        {:error, "El usuario ya pertenece al equipo #{nombre_team}."}
      else
        team_modificado = %{team | participantes: team.participantes ++ [participante.id]}
        TeamRepository.actualizar_team(team_modificado)
        {:ok, "El usuario se ha unido al equipo #{nombre_team} exitosamente."}
      end
    end
  end
